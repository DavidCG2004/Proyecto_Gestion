...
const AdminCommentsPage = ({ user }) => {
  ...
  const handleSaveComment = async (commentData) => {
    ...
    if (upsertError) setError(upsertError.message);
      ...
      displayMessage(setSuccessMessage, '¡Comentario guardado!');
      fetchCommentsAndRoutes();
    ...
    <>
      ...
      >
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Moderar Comentarios</h1>
          {user && (
            <motion.button
        ...
        {loading ? (
          ...
          <div className="text-center text-gray-500 py-16">
            ...
            <p>Aún no hay comentarios de los usuarios.</p>
          </div>
        ) : (
...
// El componente CommentFormModal permanece aquí, ya que no lo has movido.
const CommentFormModal = ({ comment, routes, onSave, onClose, loading, error }) => {
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <select
              id="route_id" name="route_id" value={formData.route_id || ''} onChange={handleChange}
              className="w-full px-4 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/30" required
            >
          ...
          <div>
            ...
            <textarea
              id="comment_text" name="comment_text" value={formData.comment_text} onChange={handleChange} rows="4"
              className="w-full px-4 py-2 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/30" required
            ></textarea>
          </div>
          <div>
          ...
          <div className="flex justify-end gap-3 mt-6">
            <motion.button
              type="button" onClick={onClose} className="px-5 py-2 bg-gray-200 text-gray-800 rounded-xl font-medium hover:bg-gray-300"
            >
              Cancelar
            </motion.button>
            <motion.button
